...
export default class AddProjects extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            remind_period: '',
            remind_message: '',
            subdivisions: [{address:"", smooth:"", total_subdivision:""}],            
            newbackgrounds:[],
            newlogoes:[],
            newgallery: [],
    ...
    async onSubmit(e) {
        e.preventDefault();
            const data = {
                ...
                newbackgrounds: this.state.newbackgrounds,
                newlogoes: this.state.newlogoes,
                newgallery: this.state.newgallery,
                url_link:this.state.url_link,
            }
           axios.post(serverUrl+'api/project/new', data)
            .then(async res => {
                console.log(res)
                this.setState({ project_id :res.data.data.id})
                await this.getTimelinesByManager(this.state.manager_id);
                await this.customerByClient(this.state.project_id);
    ...
    selectThis(folder_id, folder_name) {
        console.log('dada',folder_id,folder_name)
        axios.post(serverUrl+'api/file/getfolderfiles/'+folder_id)
        .then(response => {
            console.log('selectThis,',response)
            this.setState({
                files: response.data,
                folder_id: folder_id,
               ...
               <div className="dashboard_Table_block">
                    ...
                    {this.state.alert_message == "error" ? <ErrorAlert message={this.state.message} /> : null}
                        ...
                        <ul className="resp-tabs-list GoTabsList">
                            <li className="tab-list-item">פרטים כללים </li>{/* General details */}
                            <li   className="tab-list-item">ציר זמן</li>{/* Timeline */}
                            <li   className="tab-list-item">מסמכים</li>{/* Documents */}
                            <li   className="tab-list-item">מתעדכנים </li>{/* Are being updated */}
                        </ul>
                                     ...
                                     this.state.steps.map((step, index)=>{
                                        return (
                                            <tr key={index}>
                                                ...
                                                <td>{index+1}</td>
                                                <td>
                                                <input required value={step.step_name || ''}type="text" className="form-control"  name="step_name" onChange={this.stepName(index)}/>
                                                </td>
                            ...
                            <div className="row ">
                                ...
                                <Link to={`/managers/${btoa(this.state.manager_id)}/projects/${btoa(this.state.project_id)}/clients/add`} className="btn secondary_btn updated_btn ">
                                    ...
                                    <button className="btn Upload_btn">
                                        ...
                                        {this.state.spinn?<i class="fa fa-spinner fa-spin" aria-hidden="true"></i>:
                                         <CsvParse
                                         keys={keys}
                ...
                <div className="modal-dialog modal-dialog-centered" role="document">
                    ...
                    {this.state.alert_message == "success" ? <SuccessAlert message={this.state.message} /> : null}
                    {this.state.alert_message == "error" ? <ErrorAlert message={this.state.message} /> : null}
                    <form onSubmit={this.submitFolder} id="formFoler">
                 ...
                 {/* Modal */}
                 <div id="formFile" className="modal fade" ref={modal=> this.modal = modal} id="NewClientFormupload" tabIndex={-1} role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                <div className="modal-dialog modal-dialog-centered" role="document">
                    <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title" id="exampleModalCenterTitle">תיקייה חדשה</h5>
